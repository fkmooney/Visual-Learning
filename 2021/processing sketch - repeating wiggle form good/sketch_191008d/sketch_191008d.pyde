...
angles_1 = [random(3*x, x+a_step) for x in angles_1]
angles_2 = [random(5*x, x+a_step) for x in angles_2]
...
def draw():
    ...
    x_circle, y_circle = helper.circle_points(w/10, 50+h/10, 300, a)
    
    beginShape()
    x_circle, y_circle = helper.circle_points(200+w/5, 100+h/10, 300, 50+a)
    ...
    curveVertex(x_circle, y_circle)
    x_circle, y_circle = helper.circle_points(9*w/10, 13*h/10, 100, a)
    curveVertex(x_circle, y_circle)
    curveVertex(x_circle, y_circle)
    x_circle, y_circle = helper.circle_points(3*w/10, 20*h/10, 100, a)
    curveVertex(x_circle, y_circle)
    curveVertex(x_circle, y_circle)
    endShape()
