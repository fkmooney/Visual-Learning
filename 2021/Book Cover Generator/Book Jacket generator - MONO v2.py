# changed chunking of front cover so automatically centered
...
FRONT_font = ImageFont.truetype('VeraMono-Bold.ttf',size=450) # can also use Arial
sidefont = ImageFont.truetype('LiberationMono-Bold.ttf',size=62)
BACK_font = ImageFont.truetype('LiberationMono-Bold.ttf',size=55)
CRfont = ImageFont.truetype('LiberationMono-Bold.ttf',size=40)
...
# option to remove spaces from title
...
# Front cover text
import textwrap
para = textwrap.wrap(text_no_space, width=letters_per_line) # wraps string at certain length
current_h, pad = 100, 0 # current h starting value is vert displacement of first line
for line in para:
    w, h = img_draw.textsize(line, font=FRONT_font) # returns size of string in pixels
    img_draw.text((((dpi_total_width/2-dpi_spine_width/2)-w)/2 +
    	dpi_total_width/2+dpi_spine_width/2,
    	current_h), line, font=FRONT_font)
    current_h += h + pad
