...
from line_geometry import edges_as_sets, line_intersect, Line
    ...
    def draw(self, debug=False):
        for i, (a, b) in sorted_edges(self.edges, enum=True):
            ...
            if debug:
                ...
                #text("{}".format(i), mid.[x], mid.[y])

        for i, p in enumerate(self.pts):
            fill(0)
            textSize(6)
            if debug:
                names = "ABCDEFGHIJKLMNIOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
                text("{}".format(names[i % len(names)]), p[0], p[1])

    def __add__(self, other):
        new_edges = sorted_edges(set(self.edges) ^ set(other.edges))
        if len(new_edges) == len(self.edges) + len(other.edges):
            print("ops!")
        pts = []
