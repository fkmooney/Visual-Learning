...
wm, hm = 18, 18 # dimensões elemento    
num_areas = 7
num_elementos = 200
...
def setup():
    ...
    fill(255)
    noStroke()
    areas_a = sorted(sorteia_areas(), key=chave_ordem) # key=lambda s:s[1]
    areas_b = sorted(sorteia_areas(), key=chave_ordem)
...
def chave_ordem(sequencia):
    return sequencia[1]  * 1000 + sequencia[0]
