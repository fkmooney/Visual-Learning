...
void subdivide(ArrayList<PVector> new_points,
  ...
  if (depth >= 0) {
    ...
    nx = midx + randomGaussian() * variance/4; // allter varianceto reduce bleed
    ny = midy + randomGaussian() * variance/4; // same
...
void setup() {
  ...
  rotate(radians(45));
  stack1 = polystack(-.05*width, 0.3*height, 3*width/4, 4); // position change and radius

  stack2 = polystack(.77*width, 1.12*height, 3*width/4, 4); // same
  // stack3 = polystack(.85*width, 1.25*height, 3*width/4, 2); // same

  stacklist_add(stacklist, stack1, color(107,107,107, 3)); // change color
  stacklist_add(stacklist, stack2, color(200, 200, 200, 3)); // change color
