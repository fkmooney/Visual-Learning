...
contour_simplify = 1  # higher number = rougher,simpler render
blur_amt = 3  # 5 yields less detail than 3, can also comment out blur line 
min_canny = 100  # min value determines what is not edge, lower = more tolerant, 100 good start
max_canny = 200  # max value determines what is edge, lower = more tolerant, 200 good start
resolution = 1024
no_cv = False  
...
def find_edges(IM):
    ...
    if no_cv:
        ...
        im = np.array(IM) 
        im = cv2.GaussianBlur(im,(blur_amt,blur_amt),0)
        im = cv2.Canny(im,min_canny,max_canny) 
...
def makesvg(lines):
    ...
    for l in lines:
        ...
        # can adjust stroke width below in output
