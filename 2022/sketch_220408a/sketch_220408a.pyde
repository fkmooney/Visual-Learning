...
gravity = 0.9 # additional pull in a single direction
part_num = 60 # number of particles on screen
rate = 30
...
def setup():
    ...
    opencv = OpenCV(this, 600/2, 600/2)
    create_system()
    
def create_system():
    global particles
    particles = ParticleSystem(PVector(width / 4, 50))
...
def draw():
    ...
    if ave_flow.x == ave_flow.x: # to avoid NaN :)
        particles.run(PVector(ave_flow.x*rate+gravity, ave_flow.y*rate))
