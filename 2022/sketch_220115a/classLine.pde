class Line
{
  ...
  void drawSmooth (int tolerance)
  {
    if (middle == null) setMiddle();
    //logic to remove long lines 
    int wid_tol = width * 1/tolerance;
    int hei_tol = height * 1/tolerance;
    if (abs(start.x-end.x) < wid_tol && abs(start.y-end.y) < hei_tol)
    {
      beginShape();
