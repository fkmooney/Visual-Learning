...
# Front cover text
y_start = 460
text = img_draw.text((dpi_total_width/2+dpi_spine_width/2+70, y_start), title_copy, font=FRONT_font1, fill=text_color)
text = img_draw.text((dpi_total_width/2+dpi_spine_width/2+70, y_start+150), author_copy, font=FRONT_font2, fill=text_color)
text = img_draw.text((dpi_total_width-550, 80), "a fkm classic", font=FRONT_font3, fill=text_color)
...
# Back cover text
img_draw.multiline_text((150, 1000), back_copy, font=BACK_font, fill=text_color, spacing=16)
img_draw.multiline_text((150, 2550), copyright_text, font=CRfont, fill=text_color, spacing=16)
...
d.text((0, 0), spine_copy, font=sidefont, fill=text_color)
...
txt = Image.new('RGBA', (3300, 85))  # dimensions of layer for sideways text
d = ImageDraw.Draw(txt)
d.text((0, 0), "FKM BOOKS", font=sidefont_lower, fill=text_color)
w_left = txt.rotate(90, expand=1)  # expand: expands the output to be large enough to hold the rotated image
w_right = txt.rotate(-90, expand=1)
